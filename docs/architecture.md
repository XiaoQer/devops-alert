...
> 状态：总体设计已确认，后端阶段 1、MySQL 8.4 持久化基线和首批多源信号接入已验收。
...
当前已实现上述第 1–6 步的首版确定性链路：人工报告、Alertmanager Webhook v4、CloudEvents 1.0 结构化与 Binary 模式统一生成 SignalEvent 并维护 Alert 投影；版本化服务目录提供所有者与同环境一跳依赖；持久任务按固定门槛、15 分钟同服务窗口和一跳同症状规则创建或关联 Incident，并保存规则版本、原因码、有界事实和中文解释。当前关联规则不包含维护窗口、变更、标题相似度或学习模型。第 7 步及之后的自动取证、DiagnosisRun 自动创建、AI 和事故运营仍未实现。
