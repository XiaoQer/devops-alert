...
## 2026-08-26 告警分组与风暴收敛设计已确认

已经确认但尚未实施：

- 告警中心默认展示持久化 AlertGroup，原始 Alert 下沉为可分页成员明细；
- 首版只按服务、环境、标准化症状和 120 秒滚动窗口执行高可信确定性归组，告警来源不作为必要分组键；
- 同一告警组的待处理关联任务必须合并，避免告警风暴按成员数量线性放大任务；
- SignalEvent、Alert 和逐条事故关系继续保留，告警组不删除原始事实，也不把恢复等同于事故关闭；
- 页面必须展示真实总数、分页、截断、压缩率和风暴状态，不能用当前页长度代替总数；
- 详细范围和验收条件见 `specs/active/alert-grouping-storm-convergence.md`。

当前真实基线仍是每个不同 Alert 创建一个关联任务。101 条不同告警的压测产生 101 个成功关联任务并最终收敛到 1 个 Incident；告警列表只展示前 50 条，事故详情只展示前 100 条。上述问题不得在实施验收前标记为已修复。
