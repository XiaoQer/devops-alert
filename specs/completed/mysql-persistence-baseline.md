...
已验收。MySQL-only 连接、UTC/ORM、隔离测试、单一初始迁移、现有业务回归、统一验证和用户现有 MySQL 冒烟均已完成。
...
- 持久化、人工报告与资源读取聚焦回归 57 项通过；全部后端测试 133 项通过；
- Ruff、格式和包含迁移代码的 Mypy 检查通过；
- 统一验证 133 项测试通过，覆盖率 96.04%，Ruff、格式和 Mypy 全部通过；
- 用户现有 MySQL 8.4.10 已从确认不存在的空库受控创建 `incident_intelligence`，迁移到 `0001_mysql_initial`；
- 七张业务表全部使用 InnoDB，API `/health/ready` 返回 HTTP 200 与数据库可用。
