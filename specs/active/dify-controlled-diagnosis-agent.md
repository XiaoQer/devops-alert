...
快照中的每项证据必须以模型短别名、来源、状态、基线摘要、故障期摘要和确定性解释提供；不得返回 EvidenceItem 原始 ID、内容哈希、原始监控响应、查询文本、请求全文或任意凭据。快照不足以容纳时，平台从尾部整体移除一条证据及其摘要并标记截断，不能保留无法解释的裸别名。
...
Dify 只能返回版本化 JSON 文本；平台适配器可以剥离已闭合的推理标签与 JSON 代码围栏后解析，但不会保存或展示推理内容。随后必须通过版本化 JSON 契约：
...
- Dify 只可使用快照分配的短引用别名（证据为 `E1` 起、知识为 `K1` 起），不得复述完整 ID 或内容哈希；平台在校验前把别名还原为该 DiagnosisRun 已保存的完整身份，再执行精确匹配；
