...
class GroupingJobService(Protocol):
    def claim_batch(
        self,
        lease_owner: str,
        now: datetime,
...
class GroupingProcessor(Protocol):
    def process(self, lease: AlertGroupingJobLease) -> object: ...


class AlertGroupingRunner:
    def __init__(
        self,
        *,
        job_service: GroupingJobService,
        ...
        self._lease_owner = f"alert-grouping-runner-{uuid4().hex}"
    ...
    async def run_once(self) -> int:
        ...
        leases = await asyncio.to_thread(
            ...
            self._lease_owner,
            now,
            limit=self._settings.alert_grouping_batch_size,
            lease_seconds=self._settings.alert_grouping_lease_seconds,
        )
            ...
            try:
                ...
                try:
                    await asyncio.to_thread(
                        ...
                        lease.id,
                        self._lease_owner,
                        "alert_grouping_processing_failed",
                        self._clock().astimezone(UTC),
                    )
                except Exception:
                    self.last_cycle_error_code = "alert_grouping_cycle_failed"
        return len(leases)
    ...
    async def _wait_for_next_cycle(self) -> None:
        if self._stop_event.is_set():
            return
        interval = self._settings.alert_grouping_poll_interval_seconds
