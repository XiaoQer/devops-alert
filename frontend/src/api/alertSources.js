...
const messages = { unavailable: "告警源服务暂时不可用，本次操作结果未知", error: "告警源数据暂时不可用，请稍后重试" };
...
export function fetchAlertSources(filters = {}, options = {}) {
  ...
  return requestJson(`/api/v1/alert-sources?${params}`, { signal: options.signal }, messages);
}
export function fetchAlertSource(sourceId, options = {}) { return requestJson(`/api/v1/alert-sources/${encodeURIComponent(sourceId)}`, { signal: options.signal }, messages); }
export function fetchAlertSourceReceipts(sourceId, filters = {}, options = {}) {
  ...
  return requestJson(`/api/v1/alert-sources/${encodeURIComponent(sourceId)}/receipts?${params}`, { signal: options.signal }, messages);
}
function write(path, method, body, idempotencyKey, options = {}) {
  return requestJson(path, { method, headers: { "Content-Type": "application/json", "Idempotency-Key": idempotencyKey }, body: JSON.stringify(body), signal: options.signal }, messages);
}
