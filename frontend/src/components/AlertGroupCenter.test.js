...
describe("组优先告警中心", () => {
  it("默认展示告警组并能读取第 101 条成员和进入事故", async () => {
    ...
    fetchAlertGroupSummary.mockResolvedValue({ active_groups: 1, severe_active_groups: 1, active_alerts: 101, storm_groups: 1, resolved_groups: 0, compression_ratio: 101, peak_rate_per_minute: 101, pending_group_jobs: 0, calculated_at: "2026-08-26T08:02:00Z" });
    fetchAlertGroupOverview.mockResolvedValue({ group, reason_codes: [], source_distribution: [{ name: "生产 Alertmanager", count: 101 }], severity_distribution: [{ name: "high", count: 101 }], impacted_resources: [] });
