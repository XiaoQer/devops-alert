...
export function useAlertCenter() {
  ...
  async function loadList() {
    ...
    try {
      const response = await fetchAlerts({ state: state.value, severity: severity.value, environment: environment.value, incident_linked: linked.value, query: search.value }, { signal: listController.signal });
      if (sequence !== listSequence) return;
      alerts.value = response.items.map(toAlertListItem); listState.value = alerts.value.length ? "ready" : "empty";
      ...
      alerts.value = []; selectedId.value = null; detail.value = null; listState.value = "error"; detailState.value = "idle"; listError.value = safeMessage(error, "告警数据暂时不可用");
    }
  }

  async function selectAlert(id) { selectedId.value = id; await loadDetail(id); }
  watch([state, severity, environment, linked, search], () => { window.clearTimeout(debounceTimer); debounceTimer = window.setTimeout(loadList, 250); });
  ...
  return { state, severity, environment, linked, search, alerts, summary, selectedId, detail, listState, summaryState, detailState, listError, summaryError, detailError, loadList, loadSummary, loadDetail, selectAlert };
}
